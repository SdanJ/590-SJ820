...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct  1 10:34:48 2021
...
def NN_eval(x,submatrices):
    ...
    a1 = x.transpose()
    z2=np.matmul(submatrices[0],a1)+submatrices[1]
    a2 = S(z2)
    z3=np.matmul(submatrices[2],a2)+submatrices[3]
    a3 = S(z3)
    z4=np.matmul(submatrices[4],a3)+submatrices[5]
    a4 = S(z4)
    #out = (a4>.5)[0,:]
    return a4
...
def plot_1(xcol=1,xla='x',yla='y'):
    ...
    ax.plot(X[train_idx][:,xcol]    , YPRED_T.transpose(),'.', label='Model')
...
def plot_2(xla='y_data',yla='y_predict'):
	fig, ax = plt.subplots()
	ax.plot(Y[train_idx]  , YPRED_T.transpose(),'*', label='Training') 
	ax.plot(Y[val_idx]    , YPRED_V.transpose(),'*', label='Validation') 
	ax.plot(Y[test_idx]    , YPRED_TEST.transpose(),'*', label='Test') 
	plt.xlabel(xla, fontsize=18);	plt.ylabel(yla, fontsize=18); 	plt.legend()
	plt.show()
